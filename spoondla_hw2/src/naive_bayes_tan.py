...
class NaiveBayes(object):
    def __init__(self, train_parser, classification_type="n"):
        ...
        self.root_node = None
        self.features = None
        self.conditional_mutual_information = defaultdict()
        self.graph = OrderedDict()
        self.conditional_counts_tan = {}
        self.conditional_probabilities_tan = {}
        self.classification_type = classification_type
        self.train_parser = train_parser
        self.accuracy = 0

    def calculate_conditional_mutual_information(self):
        needed_columns = self.features
        for index_1 in xrange(0, len(needed_columns)):
            for index_2 in xrange(0, len(needed_columns)):
                ...
                self.conditional_mutual_information[(column_1, column_2)] = 0
                for col_val_1 in self.train_parser.attribute_val_map[column_1]:
                    for col_val_2 in self.train_parser.attribute_val_map[column_2]:
                        for class_val in self.train_parser.attribute_val_map[self.dependent_attribute]:
                            self.conditional_mutual_information[(column_1, column_2)] += \
                                self.joint_probabilities[(column_1, column_2)][(col_val_1, col_val_2, class_val)] * \
                                np.log2( \
                                    float(self.conditional_probabilities[(column_1, column_2)][
                                              (col_val_1, col_val_2, class_val)]) / \
                                    (self.conditional_probabilities[column_1][(col_val_1, class_val)] *
                                     self.conditional_probabilities[column_2][(col_val_2, class_val)])
                                )
        ...
        while (len(queue) > 0):
            ...
            for node_1 in nodes_in_mst:
                for index, node_2 in enumerate(queue):
                    ...
                    if self.conditional_mutual_information[(node_1, node_2)] > max_wt:
                        max_wt = self.conditional_mutual_information[(node_1, node_2)]
                        max_to_node = node_2
    ...
    def calc_tan_conditional_probabilities(self, training_data):
        ...
        for node in self.graph:
            ...
            self.conditional_counts_tan[final_key] = column_class_count_dict
        ...
        for node in self.graph:
            ...
            for key in keys:
                ...
                for val in self.train_parser.attribute_val_map[node]:
                    new_key = (val,) + key
                    total = total + \
                            self.conditional_counts_tan[final_key][new_key]

                for val in self.train_parser.attribute_val_map[node]:
                    new_key = (val,) + key
                    conditional_probability_table[new_key] = \
                        self.conditional_counts_tan[final_key][new_key] / float(total)
                    self.conditional_probabilities_tan[final_key] = conditional_probability_table
    ...
    def fit(self, train_data):
        ...
        if self.classification_type == "t":
            ...
            self.calc_tan_conditional_probabilities(train_data)
    ...
    def calculate_probabilities(self, training_data):
        ...
        for column_1 in self.train_parser.attributes[:-1]:
            for column_2 in self.train_parser.attributes[:-1]:
                column_class_count_dict = train_data.groupby(
                    [column_1, column_2, self.dependent_attribute]).size().to_dict()
                for col_val_1 in self.train_parser.attribute_val_map[column_1]:
                    for col_val_2 in self.train_parser.attribute_val_map[column_2]:
                        for class_val in self.train_parser.attribute_val_map[self.dependent_attribute]:
            ...
            for index_2 in xrange(0, len(needed_columns)):
                ...
                for class_val in self.train_parser.attribute_val_map[self.dependent_attribute]:
                    ...
                    for col_val_1 in self.train_parser.attribute_val_map[column_1]:
                        for col_val_2 in self.train_parser.attribute_val_map[column_2]:
                            total[class_val] = total.get(class_val, 0) + \
                                               self.conditional_laplacian_counts[(column_1, column_2)][
                                                   (col_val_1, col_val_2, class_val)]
                    ...
                    for col_val_1 in self.train_parser.attribute_val_map[column_1]:
                        for col_val_2 in self.train_parser.attribute_val_map[column_2]:
                            conditional_probability_dict[(col_val_1, col_val_2, class_val)] = \
                                float(self.conditional_laplacian_counts[(column_1, column_2)][
                                          (col_val_1, col_val_2, class_val)]) / total[class_val]
                    ...
                    for col_val_1 in self.train_parser.attribute_val_map[column_1]:
                        for col_val_2 in self.train_parser.attribute_val_map[column_2]:
                            joint_probability_dict[(col_val_1, col_val_2, class_val)] = \
                                float(self.conditional_laplacian_counts[(column_1, column_2)][
                                          (col_val_1, col_val_2, class_val)]) / total
    ...
    def predict(self, test_data):
        ...
        if self.classification_type == 'n':
            ...
            self.accuracy = float(correctly_classified_instances) / test_data.shape[0]
            print self.accuracy
        else:
            for data in test_data:
                final_score = defaultdict()
                ...
                for node, parents in self.graph.iteritems():
                    ...
                    cpt = self.conditional_probabilities_tan[final_key]
                    key = []
                    # exclude the class_atrtribute
                    for column in [node] + parents[:-1]:
                        attr_index = self.features.index(column)
